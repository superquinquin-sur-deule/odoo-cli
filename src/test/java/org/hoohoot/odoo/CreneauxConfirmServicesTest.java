...
class CreneauxConfirmServicesTest {

    @BeforeEach
    void setupStubs() {
        WireMockOdooResource.expect(Stub.DRAFT_SHIFTS, Stub.BUTTON_CONFIRM);
    }

    @Test
    @Launch({"creneaux", "confirm-services", "--begin-date", "20/04/2026", "--end-date", "20/07/2026"})
    void confirmsServices(LaunchResult result) {
    ...
    @Test
    @Launch(value = {"creneaux", "confirm-services", "--begin-date", "31-12-2026", "--end-date", "20/07/2026"}, exitCode = 2)
    void rejectsInvalidDateFormat(LaunchResult result) {
        assertThat(result.getErrorOutput()).contains("mauvais format de date");
    }

    @Test
    @Launch(value = {"creneaux", "confirm-services", "--begin-date", "20/07/2026", "--end-date", "20/04/2026"}, exitCode = 2)
    void rejectsBeginAfterEnd(LaunchResult result) {
        assertThat(result.getErrorOutput()).contains("--begin-date");
    }

    @Test
    @Launch(value = {"creneaux", "confirm-services", "--begin-date", "20/04/2026"}, exitCode = 2)
